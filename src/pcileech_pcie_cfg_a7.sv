...
module pcileech_pcie_cfg_a7(
    ...
    // ========================================================================
    // INTERRUPT LOGIC - Level-based INTx for SM750 shared interrupt line
    //
    // int_enable = |(raw_int_status & int_mask)  from mmio_bar_top
    //
    // PCIe legacy INTx is level-triggered:
    //   • Assert  when any masked engine source is pending  (int_enable HIGH)
    //   • Deassert when software has W1C-cleared all sources (int_enable LOW)
    //
    // The FSM tracks what the PCIe core currently believes (int_asserted)
    // and drives the Xilinx 7-series cfg_interrupt / cfg_interrupt_assert
    // handshake whenever reality diverges from that belief.
    // ========================================================================
    ...
    reg        cfg_int_assert;          // drives ctx.cfg_interrupt_assert
    reg        cfg_int_valid;           // drives ctx.cfg_interrupt
    reg        int_asserted;            // shadow: what PCIe core believes
            ...
            if ( (rw[RWPOS_CFG_CFGSPACE_STATUS_CL_EN] | rw[RWPOS_CFG_CFGSPACE_COMMAND_EN]) & ~in_cmd_read & ~in_cmd_write & ~rw[RWPOS_CFG_RD_EN] & ~rw[RWPOS_CFG_WR_EN] & ~rwi_cfg_mgmt_rd_en & ~rwi_cfg_mgmt_wr_en )
                if ( rwi_count_cfgspace_status_cl < rw[672+:32] )
                    ...
                    rw[RWPOS_CFG_WR_EN] <= 1'b1;
                        ...
                        1:begin
                        rw[RWPOS_CFG_WR_EN] <= 1'b1;
                            rw[143:128] <= 16'h010a;                            // cfg_mgmt_di: int pin/int line
                            rw[159:144] <= 16'h0000;                            // cfg_mgmt_di:  max lat / min gnt
                        rw[169:160] <= 15;                                   // cfg_mgmt_dwaddr
                        rw[170]     <= 1;                                   // cfg_mgmt_wr_readonly
                        rw[171]     <= 0;                                   // cfg_mgmt_wr_rw1c_as_rw
                            rw[172]     <= 1;   // int line
                            rw[173]     <= 1;   // int pin
                            rw[174]     <= 0; // cfg_mgmt_byte_en: status register
                            rw[175]     <= 0; // cfg_mgmt_byte_en: status register 
                            init_done <= 1;
                        end
                    endcase
                end
